...
export const DashboardPage: React.FC<DashboardPageProps> = ({ onNavigate }) => {
  ...
  const loadDashboardData = async () => {
    setIsLoading(true);
    try {
      const [coursesRes, papersRes, statsRes, facultyRes] = await Promise.all([
        api.get('/courses'),
        api.get('/papers'),
        api.get('/admin/stats').catch(() => ({ data: {} })),
        api.get('/faculty/profile').catch(() => api.get('/auth/me')).catch(() => ({ data: null }))
      ]);
      setCourses(coursesRes.data);
      setPapers(papersRes.data);
      setStats(statsRes.data);
      if (facultyRes && facultyRes.data) {
  ...
  const facultyName = faculty?.full_name || faculty?.name || 'Faculty Member';
  const facultyDept = faculty?.department || 'Department of Computer Science';
  const facultySemester = faculty?.semester || 'Semester VI';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 text-white rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="space-y-2 text-center md:text-left relative z-10">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 pt-0.5">
            <span className="bg-slate-800 text-blue-300 text-xs px-2.5 py-1 rounded-lg border border-slate-700 font-semibold">
              {facultyDept}
            </span>
            <span className="bg-slate-800 text-indigo-300 text-xs px-2.5 py-1 rounded-lg border border-slate-700 font-semibold">
              {facultySemester}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs flex items-center space-x-4 hover-card-lift">
          ...
          <div>
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">My Subjects</span>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs flex items-center space-x-4 hover-card-lift">
          ...
          <div>
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Syllabi Uploaded</span>
            <div className="text-xl font-extrabold text-slate-900 mt-0.5">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider">My Subjects</h2>
            </div>
            <button
              onClick={() => setShowCourseModal(true)}
          ...
          {isLoading ? (
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 text-center space-y-2">
              <p className="text-xs text-slate-500 font-medium">No assigned subjects found.</p>
              <button
                onClick={() => setShowCourseModal(true)}
              ...
              {courses.map((c) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[11px] font-semibold text-slate-500">{c.semester || 'VI Semester'}</span>
                  </div>
